...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Prepare unified SSS scene configs with a fixed iteration budget."
    )
    parser.add_argument(
        "--project-root",
    ...
    upstream_configs = project_root / "third_party" / "3D-student-splatting-and-scooping" / "configs"
    generated_root = validation_root / "generated_configs" / "sss"
    generated_root.mkdir(parents=True, exist_ok=True)

    for scene, template_scene in SCENE_TEMPLATES.items():
        template_path = upstream_configs / f"{template_scene}.json"
        if not template_path.is_file():
            raise FileNotFoundError(f"Missing SSS template config: {template_path}")
        ...
        # SSS loads its JSON config after parsing CLI arguments, so an upstream
        # resolution value would silently override the unified benchmark CLI.
        config["resolution"] = args.resolution

        output_path = generated_root / f"{scene}.json"
        output_path.write_text(json.dumps(config, indent=2, sort_keys=True) + "\n", encoding="utf-8")
        print(
            f"Generated unified SSS config: {output_path} "
